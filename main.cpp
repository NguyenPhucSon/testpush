...
const int SCREEN_HEIGHT = 386;

int actualMineRemain = 8;
int mineRemain = 8;
...
bool loadImage ()
{
    ...
    background = loadTexture("res/image/easy.png");
    if (background == NULL) {
...
void mark(int x, int y)
{
        if (mineMap[y][x].revealable){
            ...
            displayCounting.render(210, 33, &displayMineRemain[0]);

            if (mineRemain > 0) {
                displayCounting.render(238, 33, &displayMineRemain[mineRemain]);
            } else {
                displayCounting.render(238, 33, &displayMineRemain[0]);
...
void replay()
{
    ...
    faceP.render(126, 35, &playingFace);
        ...
        if (!loadImage()) {
            ...
            while (isRunning) {
                while (SDL_PollEvent( &mainEvent )!=0) {
                    ...
                    if (mineRemain < 0) {
                        faceL.render(126, 35, &losingFace);
                        SDL_RenderPresent(gRenderer);
                    } else {
                        faceP.render(126, 35, &playingFace);
                        SDL_RenderPresent(gRenderer);
                    ...
                    if (mainEvent.type == SDL_MOUSEBUTTONDOWN && (x>=0 && x<8 && y>=0 && y<8)) {
